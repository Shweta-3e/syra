...
function keyupFunction(e) {
    ...
    if (chatText = document.getElementById("chatText").value, 13 == e.which || 13 == e.keyCode) {
        ...
        $("#chatArea").append(a), document.getElementById("chatText").value = "", alert(document.getElementById("chatArea").offsetHeight), $(".chat-history").animate({
            scrollTop: document.getElementById("chatArea").offsetHeight
        }, 0)
    }
}
...
function getbase() {
    ...
    o *= 100, o = Math.round(o), r *= 100, (r = Math.round(r)) > 50 ? (textcol = "#000000", syraimg = "https://syra.ai/genericbot/syra.png") : (textcol = "#ffffff", syraimg = "https://syra.ai/genericbot/syrawithbackground.png")
}
...
function AuthorizeAgain() {
    var e = new XMLHttpRequest;
    e.open("POST", customerDetails.Data.BotURI + "/directline/conversations", !0), e.setRequestHeader("Content-Type", "application/json"), e.setRequestHeader("Authorization", "Bearer " + FirstToken), e.send(), e.onreadystatechange = function() {
...
function insertChat(e, t, a) {
    var i = "",
        o = windowurl();
    new Date, "https://syra.ai/#" == o ? ("me" == e && (ConversationCounter += 1, i = '<li class="right clearfix"><span class="chat-img pull-right" style="margin-right: 10px;"><img src="https://syra.ai/TaxBot_MH/Assets/Me.png" alt="User Avatar" class="img-circle" style="margin-right:15px;margin-top:33px;margin-bottom:-80px;margin-left:1000px;" height="25" width="25"/><div class="chat-body clearfix"><div class="speech-bubble-r pull-right" style="background: #ffb663;border-left-color: #ffb663">' + t + "</div></div></span></li>"), "ThirdEye" == e && (i = '<li class="left clearfix"><span class="chat-img pull-left"><img src="https://syra.ai/TaxBot_MH/Assets/MH.png" alt="User Avatar" style="margin-bottom:-35px;"class="" height="25" width="25" /></span><div class="chat-body clearfix" id="speech-bubble"><div class="speech-bubble" >' + t + "</div></div></li>")) : ("me" == e && (ConversationCounter += 1, i = '<li class="right clearfix"><span class="chat-img pull-right"><img src="https://syra.ai/TaxBot_MH/Assets/Me.png" alt="User Avatar" class="img-circle" style="margin-right:-10px;margin-top:33px;margin-bottom:-80px;margin-left:1000px;" height="28" width="25"/><div class="chat-body clearfix"><div class="speech-bubble-r pull-right">' + t + "</div></div></span></li>"), "ThirdEye" == e && (i = '<li class="left clearfix"><span class="chat-img pull-left"><img src="https://syra.ai/TaxBot_MH/Assets/MH.png" alt="User Avatar" class="" height="25" width="25" /></span><div class="chat-body clearfix" id="speech-bubble"><div class="speech-bubble" >' + t + "</div></div></li>")), setTimeout(function() {
...
function UserAction(e, t) {
    ...
    o.open("POST", "https://directline.botframework.com/v3/directline/conversations/" + convid + "/activities", !0), o.setRequestHeader("Content-Type", "application/json"), o.setRequestHeader("Authorization", "Bearer " + token), o.send(a), o.onreadystatechange = function() {
        if (4 == o.readyState && 200 == o.status) {
            JSON.parse(o.responseText);
            var e = new XMLHttpRequest;
            e.open("GET", "https://directline.botframework.com/v3/directline/conversations/" + convid + "/activities", !0), e.setRequestHeader("Authorization", "Bearer " + token), e.send(), e.onreadystatechange = function() {
                if (4 == e.readyState && 200 == e.status) {
                    var t = JSON.parse(e.responseText),
                        a = 1,
                        o = 0;
                    for (i = t.activities[a].text;
                        "" != i;) customerDetails.Data.Botchatname == t.activities[a].from.name && (o >= j && (i = "", i = t.activities[a].text, str = i.split(" "), 3 == str.length && "Image" == str[0] && (i = '<img src="images/' + str[1] + '" height="70px" alt="' + str[2] + '">'), insertChat("ThirdEye", i, 0), j++), o++), a++, i = null != t.activities[a] ? t.activities[a].text : "";
...
function chatbot() {
    var clientid = $("script").attr("clientID");
	var userreq = new XMLHttpRequest();
    userreq.open("GET", "http://147.75.71.162:8585/customer/getcustomerdetails?clientid=" + clientid,true);
    userreq.send();
    userreq.onreadystatechange = function() {
        if (userreq.readyState==4 && userreq.status==200) {
			...
			//customerDetails.Data.DomainName="";
            getbase(),
                $("head").
                    append('<head><title>TaxBot</title><meta charset="utf-8">' +
                    '<meta name="viewport" content="width=device-width, initial-scale=1">' +
                    '<link href="https://syra.ai/genericbot/TaxBot_Edited.css" rel="stylesheet">' +
            ...
            var e = '<body style="overflow-x: hidden;font-family: "Tahoma", sans-serif;" id="chatbody">' +
                '<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"><\/script>' +
                '<style>.bubble{border-radius: 5px; box-shadow: 0 0 6px #B2B2B2; line-height: 1.3;' +
                ' display: inline-block; padding: 10px 18px; position: relative; vertical-align: top;' +
                'text-align:left; font-family:Tahoma, sans-serif !important;}.bubble::before { background-color: ' +
                customerDetails.Data.BaseColor + '; content: " "; display: block; height: 16px; position: absolute;' +
                ' top: 11px; transform: rotate( 29deg ) skew( -35deg );-moz-transform:    rotate( 29deg ) skew( -35deg );' +
                ' -ms-transform:     rotate( 29deg ) skew( -35deg ); -o-transform:      rotate( 29deg ) skew( -35deg );' +
                '  -webkit-transform: rotate( 29deg ) skew( -35deg );   ' +
                ' width:  20px;\tfont-family: Tahoma, sans-serif !important;}.speech-bubble-r {position: relative;background: ' +
                customerDetails.Data.BaseColor + ';border-radius: .4em;color: black;padding-right: 15px;padding-top: 12px;' +
                'text-align: right;width: 300px;min-height: 45px; margin-top: 15px; margin-right: 10px; font-family: Tahoma,' +
                ' sans-serif !important;}.speech-bubble-r:after { content: " ";  position: absolute; right: 0; top: 50%; width: 0;' +
                ' height: 0; border: 20px solid transparent; border-left-color: ' +
                customerDetails.Data.BaseColor + ';    border-right: 0; border-bottom: 0; margin-top: -10px; margin-right: -20px;' +
                ' font-family: Tahoma, sans-serif !important;}</style>' +
                '<script type="text/javascript">function closeChat(){$("#ChatWindow").fadeOut(300);$("#chatstart").fadeToggle(); }' +
                '<\/script><script>$(document).ready(function(){$("#chatstart").click(function(){$("#ChatWindow").fadeToggle();' +
                '$("#chatstart").fadeToggle();  });});<\/script><a href="#" id="chatstart" class="float"><div class="chatbubble">' +
                '<div class="bubble you" style="background-color: ' + customerDetails.Data.BaseColor + '"><font color="'
                + textcol + '">' + customerDetails.Data.WelcomeMsg + '</font></div></div><span class="badge_taxbot" style="margin-right:28px;margin-top:5px;background-color:#f72b18;">2</span><img src="https://syra.ai/TaxBot_MH/Assets/chatbot.png" height="100%" width="100%" style="margin-left: 20px;"></a><div class="row pull-right float" style="width: 30%; height: 90%;position:fixed; min-width:350px;min-height:400px;font-size: 14px;" id="ChatWindow"><div class="col-md-12 col-sm-12 col-xs-12 col-lg-12" style="line-height: 1.5" id="ChatBodyArea"><div class="row" style="background-color: ' + customerDetails.Data.BaseColor + ';border-color: #659041;margin-left: 0px;height:35px;border-top-left-radius:10px;border-top-right-radius:10px; margin-bottom: -2px;"><div class="col-md-5 col-sm-5 col-xs-5 col-lg-5"><i class="fa fa-close" style="padding-top: 8px; font-size:20px;margin-right:120px; cursor: pointer; color:' + textcol + '" onclick="closeChat()"></i></div><div class="col-md-5 col-sm-5 col-xs-5 col-lg-5"><font color="' + textcol + '" style="float: right; padding-top: 8px; margin-right:-18px;" >Custom Chatbots by</font></div><div class="col-md-2 col-sm-2 col-xs-2 col-lg-2"><a href="https://syra.ai/" target="_blank"><img src="' + syraimg + '" height="20px" width="32px" style="float: right;margin-top:8px;margin-right:5px; " /></a></div></div><div class="panel panel-primary" style="border-color: ' + customerDetails.Data.BaseColor + ';margin-bottom:1px;margin-right:-15px;border-width:3px;font-size: 14px"><div class="panel-body" style="background-color: #FFFFFF;" id="panel-body"><ul class="chat" id="chatArea"></ul></div></div><div class="buttonInside" > <input id="btn-input" type="text" class="form-control pull-right" placeholder="Ask Your Tax Related Question..." style="width:104%;margin-right:-15px;height:50px;margin-top:-10px;background-color: #FFFFFF;border-color:' + customerDetails.Data.BaseColor + '; border-width:3px; border-top-width: 2px;" onkeypress="runScript(event)"><button id="btn-click"type="button" class="btn btn-outlined btn-primary" style="background-color:' + customerDetails.Data.BaseColor + ';border:none;margin-right: -10px;color:black;font-weight:bold;right: 12px" onclick="runScriptByClick()"><font color="' + textcol + '">ASK</font></button></div></div></div></body>';
	...
	else{alert(customerDetails.Message),$("body").append(e),text = '<font color="red"><b>'+customerDetails.Message+'</b></font>', insertChat("ThirdEye", text, 0), $("#btn-input").attr("disabled", "true"), $("#btn-click").attr("disabled", "true")}
